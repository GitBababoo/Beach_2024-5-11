    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($imageFileType, ['jpg', 'jpeg', 'png', 'gif'])) {
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                ...
                $stmt = $conn->prepare("INSERT INTO beach_content (title, description, image_path) VALUES (?, ?, ?)");
...
function deleteCard($conn, $id) {
    ...
    $stmt = $conn->prepare("SELECT image_path FROM beach_content WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // ลบข้อมูลในฐานข้อมูล
        $stmtDel = $conn->prepare("DELETE FROM beach_content WHERE id = ?");
